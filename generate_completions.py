...
def main():
    ...
    print(model_configs[args.model_name])
    ...
    test_cases = []
    for behavior_id, _test_cases in test_cases_data.items():
        test_cases.extend([dict(behavior_id=behavior_id, test_case=_test_cases)])
...
def generate_response(model_name, test_cases, image_path):
    ...
    for i in trange(0, len(test_cases), batch_size):
            ...
            [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": test_case['test_case'][1]},
                        {"type": "image", "url": os.path.join(image_path, test_case['test_case'][0])},    
                    ],
                },
            ]
            for test_case in batched_test_cases 
        ]

        responses += multi_modal_model.get_model_response(messages)
...
def load_generation_function(model_config, max_new_tokens, test_cases_path):
