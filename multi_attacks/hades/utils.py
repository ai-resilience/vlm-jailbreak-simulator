...
def load_api_keys():
    """Load API keys from configs/api_keys.yaml file"""
    api_keys_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'configs', 'api_keys.yaml')
    try:
        with open(api_keys_path, 'r') as f:
            config = yaml.safe_load(f)
        return config.get('openai_api_key', 'your api key')
    except FileNotFoundError:
        print(f"Warning: configs/api_keys.yaml not found at {api_keys_path}. Using default value.")
        return 'your api key'
    except Exception as e:
        print(f"Warning: Error reading configs/api_keys.yaml: {e}. Using default value.")
        return 'your api key'
...
def generate_caption(question):
    ...
    try:
        ...
        print(f"API Response: {res}")  # 디버깅용
        caption = extract_caption(res)
        print(f"Extracted caption: {caption}")  # 디버깅용
        if caption:
            return caption
        else:
            #return generate_caption(question)
            print(f"Caption extraction failed, using fallback")  # 디버깅용
            return f"A scene depicting {question.lower()}"
    except:
        #return generate_caption(question)
        return f"A scene depicting {question.lower()}"
...
def generate_sd_prompt(question):
    ...
    try:
        ...
        if prompt:
            ...
            time.sleep(0.2)
            #return generate_sd_prompt(question)
            return question
    except:
        time.sleep(0.2)
        #return generate_sd_prompt(question)
...
def generate_keyword(question):
    ...
    try:
        ...
        if prompt:
            ...
            time.sleep(0.2)
            #return generate_keyword(question)
            # Extract a simple keyword from the question as fallback
            words = question.lower().split()
            for word in words:
                if len(word) > 3 and word not in ['how', 'what', 'when', 'where', 'why', 'make', 'create']:
                    return word
            return "behavior"
    except:
        time.sleep(0.2)
