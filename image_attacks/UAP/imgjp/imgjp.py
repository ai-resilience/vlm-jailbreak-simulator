...
class ImgJP:
    ...
    def generate_test_cases(self, behaviors, save_dir, verbose=False):
        ...
        self.image_path = os.path.join(save_dir, 'images')
        #self.generate(image_path)
        if self.constrained:
            self.PGD_constrained(self.image_path)
        else:
            self.PGD_unconstrained(self.image_path)
        ...
        save_path = os.path.join(save_dir, 'ImgJP_test_cases.json')
    ...
    def PGD_unconstrained(self, image_path):
        ...
        img = self.model.vis_processor(img_pil).unsqueeze(0)

        adv_noise = img #torch.rand_like(img).to(self.model.device) # [0,1]
        ...
        transforms.ToPILImage()(adv_img_prompt).save('%s/bad_prompt.bmp' % (image_path))
